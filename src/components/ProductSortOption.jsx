...
function ProductSortOption() {
  ...
  const desktopOptions = [
    ...
    { label: 'Ratings', value: 'option3' },
  ];

  return (
  <>
  {/* Mobile Sort */}
    <div className="flex flex-row items-center gap-2 mb-[22px] text-[18px] leading-6 font-semibold cursor-pointer lg:hidden" onClick={handleSortToggle}>
    Sort by
    <svg
      width="40"
      height="40"
...
{isSortToggle &&
  <div className="absolute top-0 left-0 w-full h-screen bg-black opacity-50 z-10  lg:hidden transition-all delay-150 duration-300 ease-in-out" onClick={handleSortToggle}></div>
}


  <div className={`bg-white w-full fixed left-0 rounded-t-2xl px-[18px] z-20 lg:hidden ${
      isSortToggle ? "bottom-0" : "bottom-[-100%]"
    } transition-all  duration-300 ease-in-out`}>
  <div className="header-sortby flex justify-between py-8">
    <div className="text-[16px] text-[#3366FF] leading-5 font-normal cursor-pointer" onClick={handleSortToggle}>Cancel</div>
    ...
    {options.map((option, index) => (
        ...
        <div className="flex justify-center pb-[22px]">
          <button type="button" onClick={() => handleSort(selectedOption)}  className="bg-black text-white font-semibold py-[17px] px-[147px] mt-6">Apply</button>
        </div>  
      </div>
    </div>
    ...
    { matches &&
      <div className="flex flex-col justify-end dropdown relative ">
        <div className='flex items-center mb-[68px] border border-[#C1CD00]'>
          <button type='button' className="dropdown-toggle text-[16px] leading-5 font-normal ml-2.5 mr-2 my-[17px]" onClick={handleSortTogglePC} >
            Sort by
          </button>
          <svg width="15" height="9" viewBox="0 0 15 9" fill="none" xmlns="http://www.w3.org/2000/svg" className={`mr-[22px] ${isSortToggle  ? "-rotate-180 transition-all  duration-100 ease-in-out"
        ...
        <div id="options" className={`flex flex-col dropdown-content absolute top-[50%] right-0 ${ isSortToggle ? "z-10" : "z-[-1]" }  bg-white min-w-[235px] border border-[#E1E1E1]`}>
          {desktopOptions.map((option) => (
            <div key={option.value} className='flex gap-2 p-6  w-full items-center'>
              <input
                type="radio"
                id={option.value}
                name="option"
                ...
                className=' h-6 w-6 accent-[#C1CD00] text-white'
              />
              <label htmlFor={option.value} className='text-[16px] leading-5 font-normal'>{option.label}</label>
            </div>
          ))}
        </div>
      </div>
